...
export function AppHeader({ isConfigurationCollapsed, onOpenAbout, onOpenSettings, onToggleConfiguration }: AppHeaderProps) {
  ...
  const configurationToggleButtonClass = "flex h-9 w-9 shrink-0 items-center justify-center rounded-full border border-black/5 bg-white text-[var(--text-muted)] shadow-[0_12px_24px_-20px_rgba(15,23,42,0.32)] transition hover:border-[var(--primary)]/18 hover:bg-[var(--primary-soft)]/36 hover:text-[var(--primary)] xl:absolute xl:left-5 xl:top-1/2 xl:-translate-y-1/2";
  ...
  return (
    <header className="sticky top-0 z-30 flex items-center justify-between border-b border-black/5 bg-[rgba(248,249,250,0.94)] px-4 py-3 backdrop-blur md:px-5 xl:relative">
      <div className="flex items-center gap-3 xl:pl-14">
        <button
          ...
          className={configurationToggleButtonClass}
          onClick={onToggleConfiguration}
          title={isConfigurationCollapsed ? "Show configuration" : "Hide configuration"}
          type="button"
        >
          <span className="material-symbols-outlined text-[19px] rotate-90 transition-transform xl:rotate-0">
